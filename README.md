...
- 刷新、关闭标签页后数据仍在；**换浏览器或清空站点数据会丢失**，请定期使用首页 **导出 JSON** 备份。

### 数据工具与列表增强

- **导出 / 导入 JSON**：支持 **合并导入**（保留现有记录，为新 id）与 **替换导入**（清空后仅保留文件内记录）。
- **清空全部**：二次确认后删除所有记录。
- **撤销删除**：内存中保留最近 **5 次**删除，可一键恢复（刷新页面后丢失）。
- **搜索、排序、筛选**：按名称搜索；**仅本周购买**（本地周一起算）；排序：购买日 / 今日摊销高→低 / 低→高。
- **里程碑标签**：默认在满 30 / 90 / 180 / 365 天显示「已满 N 天」，可在工具栏自定义天数列表。
- **卡片扩展信息**：明日若多计一天后的日均预测、线性模型累计说明、**365 天刻度**进度条（示意用，非财务折旧）。
- **Toast 与振动**：关键操作成功/失败有底部提示；在支持的移动设备上会 **短振动** 反馈。
- **版本更新条**：部署新构建后（如 GitHub Actions 新 commit），顶部提示刷新以加载最新脚本（基于构建 id 比对）。
- **减少动效**：系统开启「减少动态效果」时，减弱背景极光动画；列表加载 spinner 亦会静止。

### 离线（PWA Service Worker）

- 当前仓库为 **零额外依赖可构建**：未内置 `vite-plugin-pwa`。若你需要 **离线可打开 / 资源缓存**，可自行接入 [vite-plugin-pwa](https://vite-plugin-pwa.netlify.app/) 并在 `main.ts` 注册 `virtual:pwa-register`（需 `npm install` 后配置）。
